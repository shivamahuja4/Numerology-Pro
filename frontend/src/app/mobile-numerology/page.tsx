...
import { Smartphone, CheckCircle, XCircle, AlertCircle, ArrowRight } from 'lucide-react';
    ...
    const handleAnalyze = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:8000/analyze/mobile', {
                ...
                body: JSON.stringify({ mobile_number: mobileNumber, dob }),
            });
        ...
        <DashboardLayout>
            <div className="max-w-3xl mx-auto space-y-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Check if your mobile number is lucky for your birth date.</p>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    <form onSubmit={handleAnalyze} className="space-y-4">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full md:w-auto px-6 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                    ...
                    <div className="space-y-6">
                        <div className={`p-6 rounded-xl border-l-4 shadow-sm flex items-start gap-4 ${getCompatibilityColor(result.compatibility)}`}>
                            ...
                            <div>
                                <h3 className="text-lg font-semibold mb-1">
                                    Your Mobile Number is {result.compatibility}
                                </h3>
                                <p className="text-sm opacity-90">
                                    Mobile Total: <span className="font-bold">{result.mobile_total}</span> •
                        ...
                        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                            ...
                            <p className="text-sm text-gray-500 mt-4">
                                Consider choosing a mobile number that sums up to one of these digits.
                            </p>
                        </div>
                    </div>
                )}
